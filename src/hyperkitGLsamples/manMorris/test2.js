...
hyperKitGL_Sheet.prototype = {
	...
	,mouseDown: function(e) {
		if(this.mouseDownXY != null) {
			...
			tmp(new hyperKitGL_XY(this.pixelRatio * (p.clientX - rect.left) * this.canvasGL.width / rect.width,this.pixelRatio * (p.clientY - rect.top) * this.canvasGL.height / rect.height));
		}
	}
...
hyperKitGLsamples_manMorris_Main.prototype = $extend(trilateral3base_TrilateralBase.prototype,{
	firstDraw: function() {
		...
		var _g = 0;
		var _g1 = connections.length;
		while(_g < _g1) {
			var i = _g++;
			c = connections[i];
			var _this = this.sketchColor;
			var x_ = c.a.x;
			var y_ = c.a.y;
			if(_this.endLine == 2 || _this.endLine == 3) {
				var _this1 = _this.contour;
				var width_ = _this.width;
				_this1.endEdges();
				if(_this1.count != 0) {
					var ax = _this1.bx;
					var ay = _this1.by;
					var radius = width_ / 2;
					var beta = -_this1.angle1 - Math.PI / 2;
					var gamma = -_this1.angle1 - Math.PI / 2 - Math.PI;
					var temp = [];
					var color = 0;
					var sides = 36;
					if(sides == null) {
					...
					while(_g2 < _g3) {
						...
						if(i1 != 0) {
							...
							_this1.pen.triangle2DFill(ax,ay,bx,by,cx,cy,color_);
						}
						angle += step1;
						bx = cx;
						by = cy;
					}
					var pA = _this1.pointsAnti.length;
					var len = temp.length / 2 | 0;
					var _g4 = 0;
					var _g5 = len + 2;
					while(_g4 < _g5) {
						var i2 = _g4++;
						_this1.pointsAnti[pA++] = temp[i2];
					}
					var pC = _this1.pointsClock.length;
					var _g6 = 1;
					var _g7 = len / 2 + 1 | 0;
					while(_g6 < _g7) {
						var i3 = _g6++;
						_this1.pointsClock[pC++] = temp[temp.length - 2 * i3];
						_this1.pointsClock[pC++] = temp[temp.length - 2 * i3 - 1];
					}
				}
			}
			_this.x = x_;
			...
			_this.contour.reset();
			var _this2 = this.sketchColor;
			var x_1 = c.b.x;
			var y_1 = c.b.y;
			var repeat = _this2.x == x_1 && _this2.y == y_1;
			if(!repeat) {
				if(_this2.widthFunction != null) {
					_this2.width = _this2.widthFunction(_this2.width,_this2.x,_this2.y,x_1,y_1);
				}
				if(_this2.colourFunction != null) {
					_this2.pen.currentColor = _this2.colourFunction(_this2.pen.currentColor,_this2.x,_this2.y,x_1,y_1);
				}
				_this2.line(x_1,y_1);
				var l1 = _this2.points.length;
				var p = _this2.points[l1 - 1];
				var l2 = p.length;
				p[l2] = x_1;
				p[l2 + 1] = y_1;
				var d1 = _this2.dim[_this2.dim.length - 1];
				if(x_1 < d1.minX) {
					d1.minX = x_1;
				}
				if(x_1 > d1.maxX) {
...
var morrisMen_MorrisConnection = function(a,b) {
	this.a = a;
	this.b = b;
};
morrisMen_MorrisConnection.__name__ = true;
...
morrisMen_MorrisNode.prototype = {
	...
	,calculateMorrisRose: function(morris) {
		...
		var v = Math.round(z2p * (16 / Math.PI));
		if(v < 0) {
			var a = v % 32;
			v = a >= 0 ? a : a + 32;
		} else if(v >= 32) {
...
morrisMen_NineMorrisBoard.prototype = {
	generate: function() {
		...
		var con = new morrisMen_MorrisConnection(ma.pos3D,mb.pos3D);
		var con1 = new morrisMen_MorrisConnection(mb.pos3D,mc.pos3D);
		var con2 = new morrisMen_MorrisConnection(ma.pos3D,mj.pos3D);
		var con3 = new morrisMen_MorrisConnection(mc.pos3D,mo.pos3D);
		var con4 = new morrisMen_MorrisConnection(mb.pos3D,me.pos3D);
		var con5 = new morrisMen_MorrisConnection(me.pos3D,mh.pos3D);
		var con6 = new morrisMen_MorrisConnection(md.pos3D,me.pos3D);
		var con7 = new morrisMen_MorrisConnection(me.pos3D,mf.pos3D);
		var con8 = new morrisMen_MorrisConnection(md.pos3D,mk.pos3D);
		var con9 = new morrisMen_MorrisConnection(mf.pos3D,mn.pos3D);
		var con10 = new morrisMen_MorrisConnection(mg.pos3D,mh.pos3D);
		var con11 = new morrisMen_MorrisConnection(mh.pos3D,mi.pos3D);
		var con12 = new morrisMen_MorrisConnection(mg.pos3D,ml.pos3D);
		var con13 = new morrisMen_MorrisConnection(mi.pos3D,mm.pos3D);
		var con14 = new morrisMen_MorrisConnection(mj.pos3D,mk.pos3D);
		var con15 = new morrisMen_MorrisConnection(mk.pos3D,ml.pos3D);
		var con16 = new morrisMen_MorrisConnection(mm.pos3D,mn.pos3D);
		var con17 = new morrisMen_MorrisConnection(mn.pos3D,mo.pos3D);
		var con18 = new morrisMen_MorrisConnection(ml.pos3D,mp.pos3D);
		var con19 = new morrisMen_MorrisConnection(mr.pos3D,mm.pos3D);
		var con20 = new morrisMen_MorrisConnection(mp.pos3D,mq.pos3D);
		var con21 = new morrisMen_MorrisConnection(mq.pos3D,mr.pos3D);
		var con22 = new morrisMen_MorrisConnection(mk.pos3D,ms.pos3D);
		var con23 = new morrisMen_MorrisConnection(mn.pos3D,mu.pos3D);
		var con24 = new morrisMen_MorrisConnection(ms.pos3D,mt.pos3D);
		var con25 = new morrisMen_MorrisConnection(mt.pos3D,mu.pos3D);
		var con26 = new morrisMen_MorrisConnection(mj.pos3D,mv.pos3D);
		var con27 = new morrisMen_MorrisConnection(mo.pos3D,mx.pos3D);
		var con28 = new morrisMen_MorrisConnection(mq.pos3D,mt.pos3D);
		var con29 = new morrisMen_MorrisConnection(mt.pos3D,mw.pos3D);
		var con30 = new morrisMen_MorrisConnection(mv.pos3D,mw.pos3D);
		var con31 = new morrisMen_MorrisConnection(mw.pos3D,mx.pos3D);
		this.connections = [con,con1,con2,con3,con4,con5,con6,con7,con8,con9,con10,con11,con12,con13,con14,con15,con16,con17,con18,con19,con20,con21,con22,con23,con24,con25,con26,con27,con28,con29,con30,con31];
	}
};
...
var trilateral3_drawing_Pen = function(paintType_) {
	...
	haxe_Log.trace("init pen ",{ fileName : "trilateral3/drawing/Pen.hx", lineNumber : 42, className : "trilateral3.drawing.Pen", methodName : "new"});
...
trilateral3_drawing_Pen.prototype = {
	triangle2DFill: function(ax,ay,bx,by,cx,cy,color) {
		...
		var cx1 = cx;
		var cy1 = cy;
		var windAdjust = this.paintType.triangle(ax1,ay1,this.z2D,bx1,by1,this.z2D,cx1,cy1,this.z2D);
		if(trilateral3_Trilateral.transformMatrix != null) {
			this.paintType.transform(trilateral3_Trilateral.transformMatrix);
		}
		if(this.useTexture) {
			ax1 /= 2000;
			ay1 /= 2000;
			...
			cx1 /= 2000;
			cy1 /= 2000;
			this.paintType.triangleUV(ax1,ay1,bx1,by1,cx1,cy1,windAdjust);
		}
		this.paintType.cornerColors(color,color,color);
		this.paintType.next();
		return 1;
	}
	,get_pos: function() {
		return this.paintType.get_pos();
	}
	,set_pos: function(v) {
		this.paintType.set_pos(v);
		return v;
	}
};
...
trilateral3_drawing_Sketch.__super__ = cornerContour_Sketcher;
trilateral3_drawing_Sketch.prototype = $extend(cornerContour_Sketcher.prototype,{
});
