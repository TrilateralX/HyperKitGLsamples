...
hyperKitGLsamples_manMorris_Main.prototype = $extend(trilateral3base_TrilateralBase.prototype,{
	firstDraw: function() {
		...
		while(_g < _g1) {
			var i = _g++;
			c = connections[i];
			this.sketchColor.moveTo(c.a.x,c.a.y);
			var _this = this.sketchColor;
			var x_ = c.b.x;
			var y_ = c.b.y;
			var repeat = _this.x == x_ && _this.y == y_;
			if(!repeat) {
				if(_this.widthFunction != null) {
					_this.width = _this.widthFunction(_this.width,_this.x,_this.y,x_,y_);
				}
				if(_this.colourFunction != null) {
					_this.pen.currentColor = _this.colourFunction(_this.pen.currentColor,_this.x,_this.y,x_,y_);
				}
				_this.line(x_,y_);
